...
class RNNCRF_Pair_Labeler(nn.Module):
    def __init__(self, input_dim, hidden_dim,  y_codebook,
                 ...
                 stopstate_symb=None, transmatrix_flag = True, to_gpu=True):
        
        super(RNNCRF_Pair_Labeler, self).__init__()
        ...
        if(interm_dim):
            ...
            self.hiddenTotag = nn.Linear(interm_dim, self.tagset_size*self.tagset_size)
        else:
            self.intermlayer = None
            self.hiddenTotag = nn.Linear(self.num_directions*hidden_dim, self.tagset_size*self.tagset_size)
            
        self.nonlinear_func = nonlinear_func     
        self.transmatrix_flag = transmatrix_flag  
    ...
    def _check_ycodebook(self, startstate_symb, stopstate_symb):
        ...
        if(stopstate_symb == None):
            ...
            if(self.transmatrix_flag): # when _STOP_ state is defined, support for additional transition parameter matrix :math:`A(y_{t-1}, y{t})` is defined 
                self.stopstate_symb = stopstate_symb
            else:
                raise(Exception('transition parameter matrix should be defined to use STOP state'))
    ...
    def compute_potential(self, seq):
        """ compute pairwise potential \phi(x_t,y_{t-1}, y_t)
        ...
        if(self.intermlayer): # in case intermediate layer is defined
            ...
            feat = self.hiddenTotag(intermres).view(-1,self.tagset_size, self.tagset_size)
                
        else:
            feat = self.hiddenTotag(rnnout).view(-1,self.tagset_size, self.tagset_size)
        # feat will have a shape (seqlen, tagset_size, tagset_size)
#         print("feat \n", feat)
        return(feat)
...
#         print("we are in compute_forward_vec")
        ...
        if(self.stopstate_symb): # when _STOP_ state is defined a transition matrix is defined too 
            offset = 2
...
#         print("backpointer \n", backpointer)
        ...
        if(self.stopstate_symb): # by default when stop state is defined, transition parameters are also defined
            t = T
