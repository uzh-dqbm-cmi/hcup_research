...
class CNNWide_Labeler(nn.Module):
    """ implements convolutional approach inspired by the one reported by `kim <https://arxiv.org/abs/1408.5882>_`
    """
    def __init__(self, y_codebook, conv_pipeline, fc_pipeline, hiddtotag_pipeline):
        super(CNNWide_Labeler, self).__init__()
        self.y_codebook = y_codebook
        self.tagset_size = len(y_codebook)
        self.conv_pipeline = self.generate_convlayers(conv_pipeline)
        self.fc_pipeline = nn.Sequential(*self.generate_layers(fc_pipeline))
        self.hiddenTotag = nn.Sequential(*self.generate_layers(hiddtotag_pipeline))
#         self._init_weights()
    ...
    def generate_convlayers(self, op_pipeline):
        conv_layers = []
        for conv_pipeline in op_pipeline:
            layers = self.generate_layers(conv_pipeline)
            conv_layers.append(nn.Sequential(*layers))
        return(nn.ModuleList(conv_layers))
    
    def forward(self, batch_seqs, tagprob=False):
        out = [conv_layer(batch_seqs).squeeze(-1).squeeze(-1) for conv_layer in self.conv_pipeline]
        out = torch.cat(out, 1)
