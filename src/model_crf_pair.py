...
class CRF_Pair_Labeler(nn.Module):
    def __init__(self, input_dim, y_codebook,
                 embed_dim=0, interm_dim=0,
                 nonlinear_func=F.relu, 
                 startstate_symb="__START__",
                 stopstate_symb=None, 
                 transmatrix_flag = True,
                 to_gpu=True):
        
        super(CRF_Pair_Labeler, self).__init__()
        ...
        if(embed_dim): # embed layer
            ...
            self.f1_dim = embed_dim
        else:
            self.embed = None
            self.f1_dim = self.input_dim

        if(interm_dim):
            # intermediate layer between hidden and tag layer
            self.intermlayer = nn.Linear(self.f1_dim, interm_dim)
            self.hiddenTotag = nn.Linear(interm_dim, self.tagset_size*self.tagset_size)
        else:
            self.intermlayer = None
            self.hiddenTotag = nn.Linear(self.f1_dim, self.tagset_size*self.tagset_size)
    ...
    def compute_potential(self, seq):
        ...
        if(self.intermlayer): # in case intermediate layer is defined
            intermres = self.nonlinear_func(self.intermlayer(seq))
            feat = self.hiddenTotag(intermres).view(-1,self.tagset_size, self.tagset_size)
                
        else:
            feat = self.hiddenTotag(seq).view(-1,self.tagset_size, self.tagset_size)
