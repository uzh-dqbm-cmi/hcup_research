...
class CRF_Labeler(nn.Module):
    def __init__(self, input_dim,  y_codebook,
        ...
        super(CRF_Labeler, self).__init__()
        ...
        if(interm_dim):
            ...
            self.hiddenTotag = nn.Linear(self.f1_dim, self.tagset_size)
    ...
    def compute_potential(self, seq):
        ...
        if(self.intermlayer): # in case intermediate layer is defined
            ...
            feat = self.hiddenTotag(seq).view(-1,self.tagset_size)
        # feat will have a shape (seqlen, 1, tagset_size)
#         print("feat \n", feat)
...
#         print("Sum P_marginal: \n", torch.sum(P_marginal, dim=1))
    ...
    def compute_refscore(self, seq_potential, y_labels):
        """ compute the probability of reference label sequence Y given observation sequence X (i.e. P(Y|X))
...
#         print("refscore \n", refscore)
    ...
    def decode_posteriorprob(self, seq_potential):
        ...
        if(self.stopstate_symb): # taking out the _STOP_ state
